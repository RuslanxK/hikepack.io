    ...
    <div className="w-full sm:p-0">
      <table className="w-full text-sm text-left text-black dark:text-gray-200">
        <thead className="text-xs text-black uppercase dark:text-gray-400">
          <tr>
            <th className="px-4 sm:px-6 py-2 border-r border-b border dark:border-gray-500 dark:text-gray-200">Color</th>
            <th className="px-4 sm:px-6 py-2 border-r border-b border dark:border-gray-500 dark:text-gray-200">Category</th>
        ...
        <tbody>
          {convertedCategories.map((category) => (
            <tr key={category.id} className="border-b border dark:border-gray-500">
              ...
              <td className="px-4 sm:px-6 py-2 font-medium text-accent whitespace-nowrap dark:text-white border-l border-r border dark:border-gray-500"> {category.name.length > 10 ? `${category.name.substring(0, 10)}...` : category.name}</td>
          ...
          {['Total', 'Worn', 'Base'].map((label, index) => {
            ...
            return (
              <tr key={label} className="text-black font-semibold border-b border dark:border-gray-500">
                <td className="px-4 sm:px-6 py-2 border-r border-b border dark:border-gray-500 dark:text-gray-200">{label}</td>
                <td className="px-4 sm:px-6 py-2"></td>
                <td className="px-4 sm:px-6 py-2 border-l border dark:border-gray-500 dark:text-gray-200">{value.toFixed(3)} {weightUnit}</td>
              </tr>
            );
          })}
        </tbody>
