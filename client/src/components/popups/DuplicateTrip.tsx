...
const DuplicateTrip: React.FC<DuplicateTripModalProps> = ({ isOpen, onClose, trip }) => {
  ...
  const [addTrip] = useMutation<AddTripData, DuplicateTripVars>(ADD_TRIP);

  const handleDeleteTrip = async () => {
    setLoading(true)

    try {
      await addTrip({ variables: { name: trip.name, about: trip.about, distance: trip.distance, startDate: trip.startDate, endDate: trip.endDate, imageUrl: trip.imageUrl }, 
