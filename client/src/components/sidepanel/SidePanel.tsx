...
const SidePanel: React.FC<SidePanelProps> = ({ isVisible, toggleVisibility, items, categories }) => {
  ...
  return (
    <div
      className={`bg-primary dark:bg-box w-1/2 sm:w-56 h-screen fixed right-0 top-0 transition-transform  ${
        isVisible ? 'translate-x-0' : 'translate-x-full'
      }`}
    >
      <button
        onClick={toggleVisibility}
        className={`fixed sm:top-0 ${isVisible ? "top-0" : "top-12"}  ${isVisible ? "left-0" : "left-[-25px]"} rounded bg-button-orange dark:bg-zinc-500 text-white p-1`}
      >
